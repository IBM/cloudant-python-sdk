...
class TestBasePageIterator(MockClientBaseCase):
  ...
  def test_get_next_resumes_after_error(self):
    page_size = 1
    mock = MockPageResponses(3*page_size, page_size)
    with patch('test_pagination_base.TestPager.operation', mock.get_next_page):
      pager: Pager = TestPageIterator(
      ...
      self.assertSequenceEqual(actual_page, mock.get_expected_page(1), "The actual page should match the expected page")
      self.assertEqual(0, pager._next_page_opts.get('start_key'), "The start_key should be 0 for the second page.")
      with patch('ibmcloudant.features.pagination._BasePager._next_request', Exception('test exception')):
        with self.assertRaises(Exception):
          pager.get_next()
      self.assertTrue(pager.has_next(), 'has_next() should return True.')
      self.assertEqual(0, pager._next_page_opts.get('start_key'), "The start_key should still be 0 for the second page.")
      ...
      self.assertTrue(pager.has_next(), 'has_next() should return False.')

  def test_get_all_restarts_after_error(self):
    page_size = 1
    mock = MockPageResponses(2*page_size, page_size)
    ...
    with patch('test_pagination_base.TestPager.operation', mockmock):
      ...
      with self.assertRaises(Exception):
        pager.get_all()
      # After the error we should not have a start_key set
      self.assertIsNone(pager._next_page_opts.get('start_key'), "The start key should be None.")
